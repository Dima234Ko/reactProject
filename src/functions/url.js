export function updateUrlWithParam(key, value, navigate) {
    ...
    navigate(`${currentUrl.pathname}?${searchParams.toString()}`, { replace: true }); // Перезаписываем URL с новым параметром, не перезагружая страницу
}
...
export function removeUrlParam(key, navigate) {
    ...
    navigate(`${currentUrl.pathname}?${searchParams.toString()}`, { replace: true }); // Перезаписываем URL с обновленными параметрами, не перезагружая страницу
  }
